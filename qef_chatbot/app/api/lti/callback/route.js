...
export async function POST(request) {
  try {
    ...
    // Use NEXTAUTH_URL without trailing slash
    ...
    // Create HTML page with auto-submit form to ensure cookie is set before redirect
    const redirectHtml = `
      <!DOCTYPE html>
      <html>
        <head>
    ...
    const response = new NextResponse(redirectHtml, {
      ...
      headers: {
        ...
        'Set-Cookie': `lti_session=${sessionToken}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${8 * 60 * 60}`,
      }
    });
    
    console.log('[LTI Callback] Cookie set via Set-Cookie header with path: /, sameSite: lax, secure: true');
